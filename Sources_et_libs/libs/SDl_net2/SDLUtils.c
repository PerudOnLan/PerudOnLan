...
void printNET2Event(SDL_Event *event)
{
  switch (NET2_GetEventType(event))
  {
  case NET2_ERROREVENT:
    printf("NET2_ERROREVENT\n");
    printf("socket  = %d\n", NET2_GetSocket(event));
    printf("message = %s\n", NET2_GetEventError(event));
    break;

  case NET2_TCPACCEPTEVENT:
    printf("NET2_ACCEPTEVENT\n");
    printf("socket  = %d\n", NET2_GetSocket(event));
    printf("port    = %d\n", NET2_GetEventData(event));
    break;

  case NET2_TCPCLOSEEVENT:
    printf("NET2_TCPCLOSEEVENT\n");
    printf("socket  = %d\n", NET2_GetSocket(event));
    break;

  case NET2_TCPRECEIVEEVENT:
    printf("NET2_TCPRECEIVEEVENT\n");
    printf("socket  = %d\n", NET2_GetSocket(event));
    break;

  case NET2_UDPRECEIVEEVENT:
    printf("NET2_UDPRECEIVEEVENT\n");
    printf("socket  = %d\n", NET2_GetSocket(event));
    break;

  default:
    printf("unknow event type (%d)\n", (int)NET2_GetEventType(event));
  }
